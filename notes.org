...
- How to use keep-around profile? A temparary profile?
  ...
  - cache
    - How to parse kept packages? just from the store path! 
      - [ ] Refactor so that kept packages do not need status (they
        shouldn't actually)
      - [ ] parse the list of store paths
      - [ ] install to cache only when not dryrunning
...
** TODO Clean errors about wtrong config options (?)
** TODO Check out if other (nix) errors are readable
** TODO Fix todos
